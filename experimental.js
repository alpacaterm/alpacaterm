...
//  }
  async query(req, cb) {
    ...
    if (req.method === "send") {
      if (req.args && req.args[0].startsWith("/")) {
        const mod = require(`./cmds/${req.args[0].slice(1)}`)
        mod(this)
      } else {
        this.ptyProcess.write(`${req.args[0]}\r`)
      }
      return
    }
...
//      chunks.push(`-ins`)
    ...
    let main_bin_path
    if (Core === "alpaca") {
      main_bin_path = platform === "win32" ? path.resolve(this.home, Core, "build", "Release", "chat") : path.resolve(this.home, Core, "chat")
    } else if (Core === "llama") {
      main_bin_path = platform === "win32" ? path.resolve(this.home, Core, "build", "Release", "main") : path.resolve(this.home, Core, "main")
    }
    if (req.full) {
      await this.exec(`${main_bin_path} ${chunks.join(" ")}`, this.cores[Core].home, cb)
    } else {
      // If ptyProcess already exists, kill it and re-initialize
      if (this.ptyProcess) {
      ...
      if (this.ins) {
        // instruct mode => different start pattern
        startpattern = /^>.*/
        endpattern = /[\r\n]>.*/
      } else {
        startpattern = /.*sampling parameters:.*/g
        endpattern = /.*mem per token.*/g
      }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ptyProcess.onData((data) => {
          ...
          if (cb) {
            if (this.html) {
              cb(this.ptyProcess, this.htmlencode(data))
            } else {
              cb(this.ptyProcess, data)
            }
          } else {
